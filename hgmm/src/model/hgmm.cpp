...
void hgmm::initial_params(){
	...
	for(int k=0;k<K;k++){
		...
		double sum=0;
		for(int c=0;c<C;c++){
			phi[k][c] = double(random() + 1.0)/(RAND_MAX + 1.0);
			sum += phi[k][c];
		}
	...
	sigma = new double[C];
	for(int c=0;c<C;c++)
		sigma[c] = double(random() + 1.0)/(RAND_MAX + 1.0);
